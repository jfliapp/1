    ...
    <el-menu :default-active="activeIndex" class="el-menu-header" mode="horizontal" :ellipsis="false"
      ...
      <div class="header-left">
        ...
        <el-sub-menu v-for="(item, i) in  menu" :key="i" :index="`${i + 10}`">
          ...
          <el-menu-item v-for="(li, k) in  item.arr" :key="k" :index="`${i + 10}-${k}`" @click="push({ name: li.name })">
            {{ li.title }}
          </el-menu-item>
        </el-sub-menu>
      </div>
...
const menu = [
  {
    ...
    arr: [
      ...
      { title: '合作夥伴', name: 'About' },
      { title: '公司概况', name: 'AboutService' },
      { title: '法律檔', name: 'AboutAptitude' },
